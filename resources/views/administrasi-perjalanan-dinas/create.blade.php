<x-app-layout :hideHeader="true">
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-[#3B5286] rounded-t-xl p-6 text-white">
            <h1 class="text-xl font-bold">Tambah Format Baru</h1>
            <p class="text-sm text-blue-100 mt-1">Isi formulir di bawah untuk menambahkan administrasi perjalanan dinas</p>
        </div>

        {{-- Form Section --}}
        <div class="bg-white rounded-b-xl shadow-lg p-8">
            <form action="{{ route('administrasi-perjalanan-dinas.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" autocomplete="off">
                @csrf
                ...
                <div class="space-y-6" x-data="{ 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nama_kegiatan" placeholder="Masukkan nama kegiatan" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 h-10">
                            @error('nama_kegiatan')
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" id="tanggal" name="tanggal" placeholder="mm/dd/yyyy" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 h-10 pl-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="jenis_perjalanan" name="id_jenis_perjalanan_dinas" x-model="jenisPerjalanan" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 h-10" placeholder="Pilih jenis perjalanan" autocomplete="off">
                                ...
                                @foreach($jenisPerjalanan as $jenis)
                                    <option value="{{ $jenis->id_jenis_perjalanan }}">{{ $jenis->nama_jenis }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <div x-show="isLuarNegeri()" style="display: none;">
                                <input type="text" name="tujuan_manual" placeholder="Masukkan negara/kota tujuan..." class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 h-10">
                            </div>

                            {{-- Hidden input to store final value (handled by controller or JS?) --}}
                            {{-- Actually, simpler to send both and let controller decide which one to use --}}
                        ...
                        <div>
                            ...
                            <select id="pelaksana" name="pelaksana[]" multiple class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 min-h-[40px]" placeholder="Pilih pelaksana..." autocomplete="off">
                                ...
                                @php
                                    ...
                                    $oldPelaksana = old('pelaksana', []);
                                @endphp

                                {{-- Master Pegawai Options --}}
                                @foreach($masterPegawai as $pegawai)
                                    <option value="{{ $pegawai->nama_lengkap }}" {{ in_array($pegawai->nama_lengkap, $oldPelaksana) ? 'selected' : '' }}>
                                        {{ $pegawai->nama_lengkap }}
                                    </option>
                                @endforeach

                                {{-- Custom Names Options (from old input) --}}
                                @foreach($oldPelaksana as $name)
