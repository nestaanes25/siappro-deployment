<x-app-layout :hideHeader="true">
    ...
    <div class="max-w-4xl mx-auto" x-data="{ showCancelModal: false }">
        ...
        <div class="bg-[#3B5286] rounded-t-xl p-6 text-white text-left">
            ...
            <p class="text-sm text-blue-100 mt-1">Isi formulir di bawah untuk menambahkan kegiatan kunjungan kerja</p>
        </div>
                ...
                <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="anggota_dewan_id[]" id="anggota_dewan_id" multiple class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 min-h-[40px]" placeholder="Pilih anggota dewan...">
                                @foreach($anggotaDewan as $ad)
                                    <option value="{{ $ad->id_anggota }}" {{ (is_array(old('anggota_dewan_id')) && in_array($ad->id_anggota, old('anggota_dewan_id'))) ? 'selected' : '' }}>{{ $ad->nama }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="rombongan[]" id="rombongan" multiple class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 min-h-[40px]" placeholder="Pilih pelaksana...">
                                @foreach($masterPegawai as $pegawai)
                                    <option value="{{ $pegawai->nama_lengkap }}" {{ (is_array(old('rombongan')) && in_array($pegawai->nama_lengkap, old('rombongan'))) ? 'selected' : '' }}>{{ $pegawai->nama_lengkap }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" id="tanggal" name="tanggal" value="{{ old('tanggal') }}" placeholder="mm/dd/yyyy" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 h-10 pl-4 bg-white" autocomplete="off">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="id_jenis_kunjungan" id="id_jenis_kunjungan" x-model="jenisKunjungan" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 h-10" placeholder="Pilih jenis kunjungan...">
                                ...
                                @foreach($jenisKunjungan as $jk)
                                    <option value="{{ $jk->id_jenis_kunjungan }}" {{ old('id_jenis_kunjungan') == $jk->id_jenis_kunjungan ? 'selected' : '' }}>{{ $jk->nama_jenis }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <div x-show="!isLuarNegeri()">
                                <select name="id_provinsi" id="id_provinsi" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 h-10" placeholder="Cari Provinsi...">
                                    ...
                                    @foreach($provinsi as $p)
                                        <option value="{{ $p->id_provinsi }}" {{ old('id_provinsi') == $p->id_provinsi ? 'selected' : '' }}>{{ $p->nama_provinsi }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div x-show="isLuarNegeri()" x-cloak style="display: none;">
                                <input type="text" name="tujuan_luar_negeri" value="{{ old('tujuan_luar_negeri') }}" placeholder="Masukkan negara/kota tujuan..." class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 h-10 px-4">
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nama_kegiatan" value="{{ old('nama_kegiatan') }}" placeholder="Masukkan nama kegiatan" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 h-10 px-4">
                        ...
                        <div>
                            ...
                            <select name="petugas_id[]" id="petugas_id" multiple class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50 min-h-[40px]" placeholder="Pilih petugas protokol...">
                                @foreach($petugasProtokol as $pp)
                                    <option value="{{ $pp->id_petugas }}" {{ (is_array(old('petugas_id')) && in_array($pp->id_petugas, old('petugas_id'))) ? 'selected' : '' }}>{{ $pp->nama }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center bg-gray-50 hover:bg-gray-100 transition cursor-pointer relative">
        ...
        <div x-show="showCancelModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm" style="display: none;">
            <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 relative" @click.outside="showCancelModal = false">
                ...
                <div class="text-center sm:text-left">
                    <h3 class="text-xl font-bold text-gray-900 mb-2">Batalkan Pembuatan Kegiatan?</h3>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Special Handle for Jenis Kunjungan update Alpine
            const tsJenis = new TomSelect('#id_jenis_kunjungan', { 
                placeholder: 'Pilih jenis kunjungan...',
                onChange: function(value) {
                    // Alpine v3: dispatch a change event on the original select so x-model picks it up
                    const nativeSelect = document.getElementById('id_jenis_kunjungan');
                    nativeSelect.value = value;
                    nativeSelect.dispatchEvent(new Event('change', { bubbles: true }));

                    // Also update Alpine component directly as a fallback
                    const form = document.querySelector('form[x-data]');
                    if (form) {
                        const alpineData = Alpine.$data(form);
                        if (alpineData) {
                            alpineData.jenisKunjungan = value;
                        }
                    }
                }
            });
