<x-app-layout :hideHeader="true">
    ...
    <div class="max-w-4xl mx-auto" x-data="{ showCancelModal: false }">
        ...
        <div class="bg-[#3B5286] rounded-t-xl p-6 text-white">
            ...
            <p class="text-sm text-blue-100 mt-1">Isi formulir di bawah untuk menambahkan kegiatan pelayanan keprotokolan</p>
        </div>

        {{-- Form Section --}}
        <div class="bg-white rounded-b-xl shadow-lg p-8">
            <form action="{{ route('pelayanan-keprotokolan.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" autocomplete="off">
                @csrf

                <div class="space-y-6">

                    {{-- ... (Anggota Dewan, Tanggal, Pukul, Jenis Kegiatan, Tempat, Nama Kegiatan, Petugas, Upload File - unchanged content) ... --}}
                    {{-- Row 1: Anggota Dewan --}}
                    <div>
                        ...
                        <select id="anggota_dewan_id" name="anggota_dewan_id[]" multiple class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50" placeholder="Pilih Anggota Dewan..." autocomplete="off">
                            @foreach($anggotaDewan as $anggota)
                                <option value="{{ $anggota->id_anggota }}" {{ (is_array(old('anggota_dewan_id')) && in_array($anggota->id_anggota, old('anggota_dewan_id'))) ? 'selected' : '' }}>
                                    {{ $anggota->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" id="tanggal" name="tanggal_kegiatan"
                                    value="{{ old('tanggal_kegiatan') }}" placeholder="Pilih tanggal"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="id_jenis_pelayanan" name="id_jenis_pelayanan"
                                class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50" placeholder="Pilih Jenis Kegiatan...">
                                <option value="">Pilih Jenis Kegiatan</option>
                                @foreach($jenisPelayanan as $jenis)
                                    <option value="{{ $jenis->id_jenis_pelayanan }}" {{ old('id_jenis_pelayanan') == $jenis->id_jenis_pelayanan ? 'selected' : '' }}>
                                        {{ $jenis->nama_jenis }}
                                    </option>
                                @endforeach
                            </select>
                            @error('id_jenis_pelayanan')
                                <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="petugas" name="petugas_id[]" multiple
                                class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50" placeholder="Pilih Petugas Protokol...">
                                <option value="">Pilih Petugas Protokol</option>
                                @foreach($petugasProtokol as $petugas)
                                    <option value="{{ $petugas->id_petugas }}" {{ (is_array(old('petugas_id')) && in_array($petugas->id_petugas, old('petugas_id'))) ? 'selected' : '' }}>
                                        {{ $petugas->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @else
                        <div class="hidden">
                            {{-- Placeholder agar grid layout tetap rapi jika dibutuhkan, atau biarkan kosong --}}
                        </div>
                        @endif
                    </div>
                    ...
                    <div>
                        ...
                        <div class="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center bg-gray-50 hover:bg-gray-100 transition cursor-pointer relative">
                            ...
                            <p id="file-name" class="text-sm text-gray-600">
                                Drag & drop file di sini atau <span class="text-blue-500 font-medium">klik untuk
                                    browse</span>
                            </p>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function getSingleSelectSettings(placeholder) {
                    return {
                        create: false,
                        sortField: {
                            field: "text",
                ...
                new TomSelect('#anggota_dewan_id', getMultiSelectSettings('Pilih Anggota Dewan...'));
                new TomSelect('#id_jenis_pelayanan', getSingleSelectSettings('Pilih Jenis Kegiatan...'));
                @if(auth()->user()->isSuperAdmin())
