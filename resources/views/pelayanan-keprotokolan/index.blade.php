...
<x-app-layout>
    {{-- Page Header Removed (Moved to Global Header) --}}
    ...
    <div x-data="{
        ...
        <div class="-mt-2 mb-10">
            ...
            <div x-show="!loading" x-cloak class="relative group">
                ...
                <a href="{{ route('pelayanan-keprotokolan', request()->except(['search', 'page'])) }}"
                    class="block relative w-60 flex-shrink-0 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-3 border border-blue-200 flex justify-between hover:shadow-md transition cursor-pointer hover:border-blue-300">
                    <div>
                        <div
                ...
                {{-- 2. Dynamic Jenis Pelayanan Cards --}}
                @foreach($jenisPelayananSummary as $jenis)
                    <a href="{{ route('pelayanan-keprotokolan', array_merge(request()->except('page'), ['search' => $jenis->nama_jenis])) }}"
                ...
                <a href="{{ route('penugasan-protokol') }}" class="block w-60 flex-shrink-0 bg-gradient-to-br from-purple-100 to-purple-200/50 rounded-xl p-3 border border-purple-200 flex justify-between hover:shadow-md transition cursor-pointer hover:border-purple-300">
                    <div>
                        ...
                        <div class="text-2xl font-bold text-gray-900">{{ number_format($totalProtokol) }}</div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
                ...
                <div class="px-6 py-4 border-b border-gray-200 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div class="flex items-center gap-3">
                        <h2 class="text-base font-bold text-gray-900">Daftar Kegiatan Pelayanan Keprotokolan</h2>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <div x-data="{ open: false, filterType: '{{ request('start_date') ? 'range' : (request('month') ? 'month' : (request('year') ? 'year' : 'range')) }}' }"
                            ...
                            <div x-show="open" x-transition:enter="transition ease-out duration-100"
                                ...
                                class="absolute top-full left-0 md:left-auto md:right-0 mt-2 w-80 bg-white rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.15)] border border-gray-100 z-50 p-4"
                                style="display: none;">
                                ...
                                <form action="{{ route('pelayanan-keprotokolan') }}" method="GET">
                                    ...
                                    <div class="flex bg-gray-100 rounded-lg p-1 mb-4">
                                        <button type="button" @click="filterType = 'range'"
                                            :class="filterType === 'range' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'"
                                            class="flex-1 py-1.5 text-xs font-medium rounded-md transition text-center">
                                            Range
                                        </button>
                                        <button type="button" @click="filterType = 'month'"
                                            :class="filterType === 'month' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'"
                                            class="flex-1 py-1.5 text-xs font-medium rounded-md transition text-center">
                                            Bulan
                                        </button>
                                        <button type="button" @click="filterType = 'year'"
                                            :class="filterType === 'year' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'"
                                            class="flex-1 py-1.5 text-xs font-medium rounded-md transition text-center">
                                            Tahun
                                        </button>
                                    </div>
                                    ...
                                    {{-- Inputs --}}
                                    <div x-show="filterType === 'range'" class="mb-4">
                                    ...
                                    <div x-show="filterType === 'month'" class="mb-4">
                                        <input type="month" name="month" value="{{ request('month') }}"
                                            :disabled="filterType !== 'month'"
                                            class="w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500">
                                    </div>

                                    <div x-show="filterType === 'year'" class="mb-4">
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <button type="submit"
                                            class="flex-1 px-3 py-2 text-xs font-medium text-white bg-[#3B5286] rounded-lg hover:bg-blue-900 transition">
                                            Terapkan
                                        </button>
                                    </div>
                                </form>
                        ...
                        {{-- Export Button & Modal --}}
                        <div x-data="{ exportOpen: false, exportFormat: 'pdf', selectAll: true, columns: { tanggal: true, waktu: true, nama_kegiatan: true, jenis_pelayanan: true, anggota_dewan: true, tempat: true } }" 
                             class="relative">
                            ...
                            <div x-show="exportOpen" 
                                ...
                                <div @click.outside="exportOpen = false"
                                    class="bg-white rounded-2xl shadow-xl w-full max-w-2xl flex flex-col max-h-[90vh] overflow-hidden transform transition-all text-left">
                                    ...
                                    <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500 mt-1">Konfigurasi kolom data dan format file untuk penarikan laporan pelayanan keprotokolan</p>
                                        </div>
                                    ...
                                    <form action="{{ route('pelayanan-keprotokolan') }}" method="GET" class="flex flex-col flex-1 overflow-hidden">
                                        ...
                                        <div class="p-6 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
                                            ...
                                            <div>
                                                ...
                                                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
                                                    ...
                                                    <label class="flex items-center gap-2 p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-300 transition cursor-pointer">
                                                        <input type="checkbox" name="columns[]" value="jenis_pelayanan" x-model="columns.jenis_pelayanan" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                                        <span class="text-sm font-medium text-gray-700">Jenis</span>
                                                    </label>
                                                    <label class="flex items-center gap-2 p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-300 transition cursor-pointer">
                                                        <input type="checkbox" name="columns[]" value="anggota_dewan" x-model="columns.anggota_dewan" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                                        <span class="text-sm font-medium text-gray-700">Anggota</span>
                                                    </label>
                                                    <label class="flex items-center gap-2 p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-300 transition cursor-pointer">
                                                        <input type="checkbox" name="columns[]" value="tempat" x-model="columns.tempat" class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                                                        <span class="text-sm font-medium text-gray-700">Tempat</span>
                                                    </label>
                                                </div>
                                            </div>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                ...
                                <th class="px-3 py-3 text-xs font-semibold text-gray-500 uppercase text-left w-[13%]">Anggota
                                    Dewan
                                </th>
                                <th class="px-3 py-3 text-xs font-semibold text-gray-500 uppercase text-left w-[22%]">Nama
                                    Kegiatan
                                </th>
                                <th class="px-3 py-3 text-xs font-semibold text-gray-500 uppercase text-left w-[13%]">Tempat
                                </th>
                                <th class="px-3 py-3 text-xs font-semibold text-gray-500 uppercase text-left w-[13%]">
                                    Protokol
                                </th>
                                <th class="px-3 py-3 text-xs font-semibold text-gray-500 uppercase text-left w-[15%]">Jenis
                                    Kegiatan
                                </th>
                        ...
                        <tbody x-show="loading" class="animate-pulse">
                            @for ($i = 0; $i < 5; $i++)
                                ...
                                <tr class="hover:bg-[#EFF6FF] transition {{ $isBoldRow ? 'bg-blue-50/30' : '' }}">
                                    ...
                                    <td class="px-3 py-4 whitespace-nowrap text-center text-xs">
                                        @php
                                            ...
                                            $date = $item->tanggal_kegiatan ? $item->tanggal_kegiatan->copy()->startOfDay() : null;
                                            $isToday = $date && $date->equalTo($today);
                                            $isFuture = $date && $date->gt($today);
                                    ...
                                    <td class="px-3 py-4 text-left">
                                        <span class="text-sm {{ $boldTextClass }}">
                                            {{ $item->jenisPelayanan?->nama_jenis ?? '-' }}
                                        </span>
                                    </td>
                                                ...
                                                <div x-data="{ open: false, top: 0, left: 0 }" class="relative">
                                                    ...
                                                    <template x-teleport="body">
                                                        <div x-show="open" x-transition:enter="transition ease-out duration-100"
                                                            ...
                                                            {{-- View --}}
                                                            <a href="{{ route('pelayanan-keprotokolan.show', $item->id_pelayanan) }}"
                                                            ...
                                                            {{-- Edit --}}
                                                            <a href="{{ route('pelayanan-keprotokolan.edit', $item->id_pelayanan) }}"
                                                            ...
                                                            {{-- Delete --}}
                                                            <button type="button"
                                                                    @click="confirmDelete('{{ route('pelayanan-keprotokolan.destroy', $item->id_pelayanan) }}')"
