<x-app-layout :hideHeader="true">
    ...
    <div class="max-w-4xl mx-auto" x-data="{ showCancelModal: false }">
        ...
        <div class="bg-[#3B5286] rounded-t-xl p-6 text-white">
            ...
            <p class="text-sm text-blue-100 mt-1">Perbarui data kegiatan dan penugasan protokol</p>
        </div>
            ...
            <form autocomplete="off" action="{{ route('pelayanan-keprotokolan.update', $item->id_pelayanan) }}" method="POST"
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="anggota_dewan_id" name="anggota_dewan_id[]" multiple
                            ...
                            @foreach($anggotaDewan as $anggota)
                                <option value="{{ $anggota->id_anggota }}"
                                    {{ (is_array(old('anggota_dewan_id')) && in_array($anggota->id_anggota, old('anggota_dewan_id'))) || (isset($item) && $item->anggotaDewan->contains($anggota->id_anggota)) ? 'selected' : '' }}>
                                    {{ $anggota->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" id="tanggal" name="tanggal_kegiatan"
                                    value="{{ old('tanggal_kegiatan', $item->tanggal_kegiatan ? $item->tanggal_kegiatan->format('Y-m-d') : '') }}"
                        ...
                        <div>
                            ...
                            <input type="text" id="waktu" name="waktu"
                                value="{{ old('waktu', $item->waktu ? \Carbon\Carbon::parse($item->waktu)->format('H:i') : '') }}"
                                placeholder="--:--"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="id_jenis_pelayanan" name="id_jenis_pelayanan"
                                class="w-full border-gray-300 rounded-lg shadow-sm focus:border-[#3B5286] focus:ring focus:ring-[#3B5286] focus:ring-opacity-50" placeholder="Pilih Jenis Pelayanan...">
                                <option value="">Pilih Jenis Kegiatan</option>
                                @foreach($jenisPelayanan as $jenis)
                                    <option value="{{ $jenis->id_jenis_pelayanan }}"
                                        {{ old('id_jenis_pelayanan', $item->id_jenis_pelayanan) == $jenis->id_jenis_pelayanan ? 'selected' : '' }}>
                                        {{ $jenis->nama_jenis }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select id="petugas" name="petugas_id[]" multiple
                                ...
                                @foreach($petugasProtokol as $petugas)
                                    <option value="{{ $petugas->id_petugas }}"
                                        {{ (is_array(old('petugas_id', $item->petugas->pluck('id_petugas')->toArray())) && in_array($petugas->id_petugas, old('petugas_id', $item->petugas->pluck('id_petugas')->toArray()))) ? 'selected' : '' }}>
                                        {{ $petugas->nama }}
                                    </option>
                                @endforeach
                            </select>
